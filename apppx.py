...
if uploaded_file is not None:
    ...
    display_cols = [c for c in ['TARIH', 'SATILAN CİNSİ', 'İSİM', 'TC', 'GİDEN HAVALE TUTARI', 'ÖDEME ŞEKLİ', 'ALTIN GRAM', 'BİRİM FİYAT', 'ADRES', 'TEL'] if c in df_valid.columns]
    st.dataframe(df_valid[display_cols])
...
<style>
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .tarih-val { position: absolute; top: 15px; right: 25px; font-size: 9pt; font-weight: bold; }
    ...
    .signature-area { position: absolute; bottom: 20px; right: 15px; width: 220px; text-align: center; }
    .signature-title { font-size: 9.5pt; font-weight: bold; text-decoration: underline; margin-bottom: 6px; }
    .signature-line { height: 15px; margin-bottom: 4px; }
    .contact-info-area { font-size: 8.5pt; text-align: left; line-height: 1.4; margin-top: 8px; font-weight: bold; }
...
"""
        ...
        for index, row in df_valid.iterrows():
            ...
            tarih_val = str(row['TARIH']) if 'TARIH' in row and pd.notna(row['TARIH']) else ""
            adres_val = str(row['ADRES']) if 'ADRES' in row and pd.notna(row['ADRES']) else ""
            tel_val = str(row['TEL']) if 'TEL' in row and pd.notna(row['TEL']) else ""
            
            tek_panel = f"""
            <div class="panel">
                <div class="header-top">
                    <div class="tarih-val">{tarih_val}</div>
                </div>
                ...
                <div class="signature-area">
                    <div class="signature-title">imza</div>
                    <div class="signature-line"></div>
                    <div class="contact-info-area">
                        <div class="contact-row">Adres: {adres_val}</div>
                        <div class="contact-row">Tel: {tel_val}</div>
                    </div>
                </div>
            </div>
            """
        ...
        st.success("İşlem tamam! Tarih, imza, adres ve telefon alanları eklenerek matbaa katmanı hazırlandı.")
